...
##############################################
# OpenGL 
SHADER_OPENGL_SRC_DIR="shaders/opengl"
OUTPUT_OPENGL_DIR="build/out/shaders/opengl"
...
find ${SHADER_OPENGL_SRC_DIR} -type f | while read f; do
    ...
    cp "$f" "${f_output_path}"
    # COMPILE_LOG="$(${GLSLANGVALIDATOR_BIN} -G --target-env opengl -o "${f_output_path}" "$f" 2>1)"
    # if [[ $? != 0 ]]; then
